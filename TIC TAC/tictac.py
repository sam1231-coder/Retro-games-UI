...
# pygame.display.set_icon(X_IMG)
O_IMG = pygame.image.load("O.png")
# pygame.display.set_icon(O_IMG)
    ...
    for i in range(3):
        for j in range(3):
            if board[i][j] == "X":
                ...
                graphical_board[i][j][1] = ximg.get_rect(
                    center=(j * 300 + 150, i * 300 + 150)
                )
            elif board[i][j] == "O":
                graphical_board[i][j][0] = oimg
                graphical_board[i][j][1] = oimg.get_rect(
                    center=(j * 300 + 150, i * 300 + 150)
                )
...
def add_XO(board, graphical_board, to_move):
    ...
    converted_x = (current_pos[0] - 65) / 835 * 2  # to fit X n O  in the image
    converted_y = current_pos[1] / 835 * 2
    if (
        board[round(converted_y)][round(converted_x)] != "O"
        and board[round(converted_y)][round(converted_x)] != "X"
    ):
...
def check_win(board):
    ...
    for row in range(0, 3):
        if (board[row][0] == board[row][1] == board[row][2]) and (
            ...
            winner = board[row][0]
            for i in range(0, 3):
                graphical_board[row][i][0] = pygame.image.load(f"Winning {winner}.png")
    ...
    for col in range(0, 3):
        if (board[0][col] == board[1][col] == board[2][col]) and (
            ...
            winner = board[0][col]
            for i in range(0, 3):
                graphical_board[i][col][0] = pygame.image.load(f"Winning {winner}.png")
    ...
    if (board[0][0] == board[1][1] == board[2][2]) and (board[0][0] is not None):
        winner = board[0][0]
        graphical_board[0][0][0] = pygame.image.load(f"Winning {winner}.png")
        SCREEN.blit(graphical_board[0][0][0], graphical_board[0][0][1])
        graphical_board[1][1][0] = pygame.image.load(f"Winning {winner}.png")
        SCREEN.blit(graphical_board[1][1][0], graphical_board[1][1][1])
        graphical_board[2][2][0] = pygame.image.load(f"Winning {winner}.png")
    ...
    if (board[0][2] == board[1][1] == board[2][0]) and (board[0][2] is not None):
        winner = board[0][2]
        graphical_board[0][2][0] = pygame.image.load(f"Winning {winner}.png")
        SCREEN.blit(graphical_board[0][2][0], graphical_board[0][2][1])
        graphical_board[1][1][0] = pygame.image.load(f"Winning {winner}.png")
        SCREEN.blit(graphical_board[1][1][0], graphical_board[1][1][1])
        graphical_board[2][0][0] = pygame.image.load(f"Winning {winner}.png")
